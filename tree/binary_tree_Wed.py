...
class BTNode:
	...
	def __repr__(self): # 트리의 노드를 문자열 표현 - print함수을 호출 
...
def postorder(node): # 후위순회 : 왼쪽 순회 -> 오른쪽 순회-> 노드 방문
		if node is None: # 재귀호출 종료
			return
		postorder(node.left)
...
def count_leaves(node): # 이진트리의 단말 노드 수 = 왼쪽 서브트리의 단말 노드 수 + 오른쪽 서브트리의 노드 수 + 1
		# 재귀적 정의 : 트리가 비어있는 경우 0, 단말노드이면 1, 아니면 왼쪽과 오른쪽 서브트리의 노드 수 +1
		if node is None: #재귀호출 종료
				return 0
		if is_leaf(node):
				return 1
...
def count_edges(root): # 이진트리의 간선의 수
...
def count_none_links(root): # 이진트리에서 노드 수가 N이면 연결되지않은 링크 수 계산
		# N = count_nodes(root)
		# 연결되지 않은 링크 수 = 전체 노드 수 - 간선 수 = N - (N-1)
		if root is None:
				return 1 # None 링크 한개
