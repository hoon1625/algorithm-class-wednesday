...
#=====================================================
# 1. k번째 작은수 찾는 문제 (kth smallest selection problem)
# - 최소 정복으로 설계, 퀵 정렬 분할 함수 이용, 재귀함수로 처리
...
def quick_select(A, left, right, k):
    ...
    if k + left == pos + 1 :         # case 1 - 피벗이 k번째인 경우
        return A[pos]
    elif k + left < pos + 1:         # case 2 - k번째 작은수가 왼쪽에 있는 경우
        return quick_select(A, left, pos - 1, k)
    else:                            # case 3 - k번째 작은수가 오른쪽에 있는 경우
...
def merge(A, left, mid, right):
    ...
    while i <= mid and j <= right :
        if A[i] <= A[j] :
            ...
            j += 1
            k += 1
    # 4. 왼쪽 부분 리스트가 남아 있는 경우
    while i <= mid:
        sorted_list[k] = A[i]
        i += 1
        k += 1
    # 4-2. 오른쪽 부분 리스트에 요소가 남아 있는 경우
    while j <= mid:
        sorted_list[k] = A[j]
        j += 1
        k += 1
...
def fib_dp_mem(n):          #fib(n)=?
    if mem[n] is None:
        if n < 2 :          # 재귀함수 호출 종료
            ...
            mem[n]   = fib_dp_mem(n-1)  +  fib_dp_mem(n-1)
